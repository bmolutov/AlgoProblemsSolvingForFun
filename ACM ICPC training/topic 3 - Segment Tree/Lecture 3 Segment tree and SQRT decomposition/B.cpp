...
struct SegmentTree {
        ...
        void set (int i, int v, int x, int lx, int rx) {
            if (rx - lx == 1) {
                ...
                tree[x].ff = :: min (tree[2 * x + 1].ff, tree[2 * x + 2].ff);
                tree[x].ss = :: max (tree[2 * x + 1].ss, tree[2 * x + 2].ss);
            }
        }
        ...
        int max (int l, int r, int x, int lx, int rx) {
            ...
            int m = (lx + rx) / 2;
            int lMax = max (l, r, 2 * x + 1, lx, m);
            int rMax = max (l, r, 2 * x + 2, m, rx);
...
signed main() {
    ...
    for (int i = 0; i < k; i++) {
        ...
        if (x > 0) {
            y++;
            cout << st.max (x, y) - st.min (x, y) << endl;
        } else if (x < 0) {
            st.set (abs (x), y);
        }
    }
